...
public final class MarkupTextAreaElement extends StyledElement<MarkupTextAreaElement> {
    ...
    protected void renderContent(Frame frame, Rect area, RenderContext context) {
        ...
        int initialLineNumberWidth = showLineNumbers
                ? String.valueOf(parsedLines.size()).length() + lineNumberSeparator.length()
                : 0;
        int textWidthWithoutScrollbar = Math.max(0, contentArea.width() - initialLineNumberWidth);
        int visualLineCountWithoutScrollbar = visualLineCount(parsedLines, textWidthWithoutScrollbar);
        boolean reserveScrollbarSpace = scrollBarPolicy == ScrollBarPolicy.ALWAYS
                || (scrollBarPolicy == ScrollBarPolicy.AS_NEEDED
                    && visualLineCountWithoutScrollbar > contentArea.height());
        ...
        int visualLineCount = visualLineCount(parsedLines, textRenderArea.width());
        int lineNumberWidth = showLineNumbers
                ? String.valueOf(visualLineCount).length() + lineNumberSeparator.length()
                : 0;
        ...
        int visibleLines = Math.min(Math.max(visualLineCount - scrollRow, 0), textContentArea.height());
    ...
    private int visualLineCount(List<Line> lines, int maxWidth) {
        if (lines.isEmpty()) {
            return 0;
        }
        if (maxWidth <= 0) {
        ...
        for (Line line : lines) {
            if (line.width() <= maxWidth
                    || (effectiveOverflow != Overflow.WRAP_CHARACTER
                        && effectiveOverflow != Overflow.WRAP_WORD)) {
                count++;
            } else if (effectiveOverflow == Overflow.WRAP_WORD) {
                count += wrappedWordLineCount(line, maxWidth);
            } else {
                count += Math.max(1, (line.width() + maxWidth - 1) / maxWidth);
            }
        }
        return count;
    }

    private int wrappedWordLineCount(Line line, int maxWidth) {
        String text = line.rawContent();
        ...
        int[] cpWidths = new int[cpCount];
        int offset = 0;
        for (int i = 0; i < cpCount; i++) {
            cpOffsets[i] = offset;
            int codePoint = text.codePointAt(offset);
            cpWidths[i] = CharWidth.of(codePoint);
        ...
        while (pos < cpCount) {
            ...
            if (lineEnd < cpCount && lineEnd > 0) {
                int previousCodePoint = text.codePointAt(cpOffsets[lineEnd - 1]);
