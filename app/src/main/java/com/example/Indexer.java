...
public class Indexer {
    ...
    public void indexEntries() throws IOException {
        List<Definition> entries = readJSON();
        index = readIndex(entries);
    }

    public List<SearchResult> searchTerm (String search) throws IOException, ParseException {
    ...
    private IndexResult readIndex(List<Definition> entries) throws IOException {
        ...
        try (IndexWriter writer = new IndexWriter(directory, new IndexWriterConfig(analyzer))) {
            for (Definition def : entries) {
                ...
                doc.add(new TextField(DEFSTR, def.getDefinition(), Field.Store.YES));
    ...
    private List<SearchResult> search(String search, ByteBuffersDirectory directory, StandardAnalyzer analyzer) throws IOException, ParseException {
        List<SearchResult> searchresults = new ArrayList<>();
        DirectoryReader reader = DirectoryReader.open(directory);
        IndexSearcher searcher = new IndexSearcher(reader);
        MultiFieldQueryParser parser = new MultiFieldQueryParser(new String[]{"term", DEFSTR}, analyzer);
        ...
        for (ScoreDoc hit : results.scoreDocs) {
            ...
            searchresults.add(new SearchResult(doc.getValues("location"), doc.getValues("term"), doc.getValues(DEFSTR)));
